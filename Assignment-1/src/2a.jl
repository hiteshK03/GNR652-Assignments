...
function ridge(X, Y, B, parameter)
...
parameter = 0.001
learningRate = 0.001
intialCost = ridge(X, Y, B, parameter)
...
learningRate = 0.001
parameter = 10^-6
newB, costHistory = gradientDescent(X, Y, B, parameter, learningRate, 30000)

# newB, costHistory = gradientDescent(X, Y, B, learningRate, 100000)
# for i in 1:numEpoch
#     println("Epoch : ",i)
#     parameter = parameter*2
#     println("Current parameter : ",parameter)
#     newB, costHistory = gradientDescent(X, Y, B, parameter*i*10, learningRate, 30000)
#     println("Validation")
#     rms = validate(newB)
#     RMSEHistory[i] = rms
# end
...
CSV.write("./data/2a.csv", DataFrame(Y_pred), writeheader=false)
